...
| 48 | Stronger decoder-only transformer | Replace or materially strengthen the decoder-only implementation beyond copy-output repair enough to generalize on retrieval tasks, then rerun the same fair comparison. |
| 49 | Larger or error-aware witnesses | Explore larger qubit witnesses or mitigation analysis after downstream and replication evidence justify the added complexity. |
...
The next promotion gate is documented in [Next transformer benchmark roadmap](docs/research/q-rope-next-transformer-benchmark-roadmap-v1.md). Stage 49 shows that copy-output repair can expose exact-offset retrieval for `rope_relative`, but PhaseWrap does not lead that repaired lane and phase-cued retrieval remains weak. The next useful work is a materially stronger decoder-only implementation beyond copy-output repair before positional-method claims are expanded.

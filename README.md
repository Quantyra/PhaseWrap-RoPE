...
| 78 | Stronger decoder-only transformer | Move beyond compact support/copy heads; preserve recovered support and use it for token selection before evaluating positional-method promotion. |
| 79 | Larger or error-aware witnesses | Explore larger qubit witnesses or mitigation analysis after downstream and replication evidence justify the added complexity. |
...
The next promotion gate is documented in [Next transformer benchmark roadmap](docs/research/q-rope-next-transformer-benchmark-roadmap-v1.md). Stage 79 shows that support-complete exposure recovers held-out support labels but still does not repair phase-cued token retrieval. The next useful work is moving beyond compact support/copy heads to a matched decoder-only mechanism that preserves recovered support and uses it for token selection before evaluating positional-method differences.

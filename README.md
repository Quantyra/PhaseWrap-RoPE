...
| 49 | Stronger decoder-only transformer | Replace or materially strengthen the decoder-only implementation beyond the current learned pointer-generator enough to generalize on retrieval tasks, then rerun the same fair comparison. |
| 50 | Larger or error-aware witnesses | Explore larger qubit witnesses or mitigation analysis after downstream and replication evidence justify the added complexity. |
...
The next promotion gate is documented in [Next transformer benchmark roadmap](docs/research/q-rope-next-transformer-benchmark-roadmap-v1.md). Stage 50 shows that the fixed-copy Stage 49 repair does not survive a learned pointer-generator decoder under the default fair audit. The next useful work is a materially stronger decoder-only implementation beyond the current pointer-generator before positional-method claims are expanded.

...
| 52 | Stronger decoder-only transformer | Redesign beyond the Stage 53 two-block hardening path enough to generalize on retrieval tasks, then rerun the same fair comparison. |
| 53 | Larger or error-aware witnesses | Explore larger qubit witnesses or mitigation analysis after downstream and replication evidence justify the added complexity. |
...
The next promotion gate is documented in [Next transformer benchmark roadmap](docs/research/q-rope-next-transformer-benchmark-roadmap-v1.md). Stage 53 shows that simple exposure/training hardening of the two-block path still leaves retrieval generalization at zero. The next useful work is a redesign beyond this autograd two-block hardening path before positional-method claims are expanded.

...
| 61 | Stronger decoder-only transformer | Redesign beyond Stage 62 enough to establish learned capacity before evaluating positional-method promotion. |
| 62 | Larger or error-aware witnesses | Explore larger qubit witnesses or mitigation analysis after downstream and replication evidence justify the added complexity. |
...
The next promotion gate is documented in [Next transformer benchmark roadmap](docs/research/q-rope-next-transformer-benchmark-roadmap-v1.md). Stage 62 shows longer support-complete training improves fit but still does not establish learned decoder capacity. The next useful work is a stronger matched learned decoder that first fits the row family without copy/lookup outputs, then evaluates positional-method differences.

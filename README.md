...
| 79 | Stronger decoder-only transformer | Replace the hard-coded support-to-token routing with learned matched decoder behavior before evaluating positional-method promotion. |
| 80 | Larger or error-aware witnesses | Explore larger qubit witnesses or mitigation analysis after downstream and replication evidence justify the added complexity. |
...
The next promotion gate is documented in [Next transformer benchmark roadmap](docs/research/q-rope-next-transformer-benchmark-roadmap-v1.md). Stage 80 shows that explicit support-to-token routing repairs phase-cued retrieval, but the repair is method-nonspecific and also solves for `no_position`. The next useful work is replacing that hard-coded routing with matched learned decoder behavior before evaluating positional-method differences.

...
| 66 | Stronger decoder-only transformer | Redesign beyond Stage 67 enough to improve original retrieval-row generalization before evaluating positional-method promotion. |
| 67 | Larger or error-aware witnesses | Explore larger qubit witnesses or mitigation analysis after downstream and replication evidence justify the added complexity. |
...
The next promotion gate is documented in [Next transformer benchmark roadmap](docs/research/q-rope-next-transformer-benchmark-roadmap-v1.md). Stage 67 shows the current two-block pointer-generator can solve a standard visible content-key retrieval row family, but it solves for `no_position` too and does not repair the original phase-cued/exact-offset retrieval blocker. The next useful work is a stronger matched learned decoder or original-row mechanism that improves retrieval generalization before evaluating positional-method differences.

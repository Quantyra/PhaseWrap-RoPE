...
| 42 | Matched decoder-only transformer or bounded plateau | Move the fair comparison into a stronger decoder-only transformer, or record that compact diagnostics have reached a bounded plateau. |
| 43 | Larger or error-aware witnesses | Explore larger qubit witnesses or mitigation analysis after downstream and replication evidence justify the added complexity. |
...
The next promotion gate is documented in [Next transformer benchmark roadmap](docs/research/q-rope-next-transformer-benchmark-roadmap-v1.md). Stage 43 shows that generator-target hardening improves the weak Stage 42 generator branch, but the compact diagnostic remains RoPE-favorable and copy-route dependent. The next useful work is a stronger matched decoder-only transformer or an explicit bounded-plateau decision, not another claim-broadening diagnostic.

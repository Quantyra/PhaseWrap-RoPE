...
| 54 | Stronger decoder-only transformer | Redesign beyond the Stage 55 metadata upper bound enough to infer retrieval cues from standard inputs, then rerun the same fair comparison. |
| 55 | Larger or error-aware witnesses | Explore larger qubit witnesses or mitigation analysis after downstream and replication evidence justify the added complexity. |
...
The next promotion gate is documented in [Next transformer benchmark roadmap](docs/research/q-rope-next-transformer-benchmark-roadmap-v1.md). Stage 55 shows that the rows are solvable only when explicit cue metadata is handed to every method, including `no_position`. The next useful work is a matched decoder redesign that infers those cues from standard inputs before positional-method claims are expanded.

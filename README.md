...
| 72 | Stronger decoder-only transformer | Redesign beyond Stage 73 enough to improve original retrieval-row generalization before evaluating positional-method promotion. |
| 73 | Larger or error-aware witnesses | Explore larger qubit witnesses or mitigation analysis after downstream and replication evidence justify the added complexity. |
...
The next promotion gate is documented in [Next transformer benchmark roadmap](docs/research/q-rope-next-transformer-benchmark-roadmap-v1.md). Stage 73 shows that the held-out phase-cued support miss is not repaired by the predeclared fixed period-pair grid. The next useful work is a stronger matched learned decoder or original-row mechanism that specifically improves phase-cued retrieval generalization before evaluating positional-method differences.

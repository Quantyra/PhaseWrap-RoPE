...
| 76 | Stronger decoder-only transformer | Redesign beyond Stage 77 enough to preserve visible-cue recovery inside a matched decoder-only mechanism before evaluating positional-method promotion. |
| 77 | Larger or error-aware witnesses | Explore larger qubit witnesses or mitigation analysis after downstream and replication evidence justify the added complexity. |
...
The next promotion gate is documented in [Next transformer benchmark roadmap](docs/research/q-rope-next-transformer-benchmark-roadmap-v1.md). Stage 77 shows that auxiliary support supervision still does not preserve the Stage 75 phase-cued repair. The next useful work is explaining the same-seed support generalization failure or moving to a matched decoder-only mechanism that preserves visible-cue recovery before evaluating positional-method differences.

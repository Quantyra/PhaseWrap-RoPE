...
| State | Expected key under `q1q0` | Dominant fraction | Wilson 95% lower bound |
| --- | --- | ---: | ---: |
| `00` | `00` | 0.959 | 0.944852 |
| `01` | `10` | 0.949 | 0.933564 |
| `10` | `01` | 0.960 | 0.945990 |
| `11` | `11` | 0.934 | 0.916890 |
...
![Full IBM Fez replacement hardware metrics](figures/qrope-full-replacement-metrics-v1.png)
...
- Add direct classical periodic-feature comparators to retrieval-stage baselines, including a five-frequency Fourier map and tiny mod-24 lookup, so the report distinguishes where the wrapped form helps from where an exposed periodic table is sufficient.
- Test variable, learned, or multi-scale PhaseWrap periods beyond the fixed 8/12 design to measure whether adaptive wrapping reduces aliasing on longer contexts without losing auditability.
- Report calibration tradeoffs more visibly across substitution stages, including top-1, MRR, ECE, and Brier score, with Stage 219 separated as the current RoPE-substitution calibration-degradation case.
- Repeat the full-replacement hardware protocol on additional devices, dates, shot budgets, and optionally error-mitigation variants before making any broader hardware robustness statement.
- Improve researcher usability with type hints and docstrings for the core scoring functions, a one-page executive summary linking the key decision tables, and a minimal Hugging Face scoring-kernel example.
...
- `scripts/run_stage84_support_auxiliary_pointer_generator_audit.py`
- `scripts/run_stage216_full_replacement_merged_result_counts.py`
- `scripts/run_stage217_full_replacement_calibration_validation.py`
...
- `logs/automated_stage_gates/stage84_support_auxiliary_pointer_generator_audit/manifest.json`
...
- `docs/publication/figures/qrope-full-replacement-metrics-v1.png`

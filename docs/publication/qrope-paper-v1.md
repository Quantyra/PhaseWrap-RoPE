...
| 2 | Stronger standard retrieval tasks | Stage 26 adds an explicit content-key QA retrieval packet, Stage 27 trains a compact attention bridge on that packet, Stage 28 trains a compact attention bridge on non-phase-cued RULER-style retrieval rows, and Stage 29 shows period-pair swaps alone do not solve the fixed-score retrieval gap. Next, move this structure into a trained small-transformer Needle-in-Haystack, RULER-style multi-hop/aggregation, or compact natural-language QA task with matched compute and multiple seeds. |
| 3 | Comparable PhaseWrap mechanism | Stage 13 through Stage 29 now test adapters and period-pair alternatives around PhaseWrap features and show mixed behavior: useful positional ranking and compact QA results, but not sufficient for the learned long-context value-output path or fixed-score retrieval. |

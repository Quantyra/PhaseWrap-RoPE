...
- Stage 6, Stage 7, Stage 8, Stage 9, Stage 10, Stage 12, Stage 13, Stage 14, Stage 15, Stage 16, Stage 17, Stage 18, Stage 19, Stage 20, Stage 21, and Stage 22 are toy downstream, retrieval, compact trained positional-attention, positional-adapter, attention-readout, learned attention-readout, stability, value-output capacity/hardening, or very small transformer experiments, not production transformer evaluations.
...
| 2 | Stronger standard retrieval tasks | Stage 22 now extends the deterministic local passkey, multi-needle, and aggregation-style packet through 4096-token contexts and is negative for fixed PhaseWrap 8/12. Next, move to trained small-transformer Needle-in-Haystack, RULER-style multi-hop/aggregation, or compact natural-language QA tasks with matched compute and multiple seeds. |
| 3 | Comparable PhaseWrap mechanism | Stage 13 through Stage 21 now test adapters around PhaseWrap features and show that explicit distance terms are needed on exact-offset retrieval and value readout. Stage 21 confirms that the current hardened positional comparison favors RoPE-like scoring across five initialization seeds. |

...
- Stage 6, Stage 7, Stage 8, Stage 9, Stage 10, Stage 12, Stage 13, Stage 14, Stage 15, Stage 16, Stage 17, Stage 18, Stage 19, Stage 20, Stage 21, Stage 22, Stage 23, Stage 24, and Stage 25 are toy downstream, retrieval, compact trained positional-attention, positional-adapter, attention-readout, learned attention-readout, stability, value-output capacity/hardening, or very small transformer experiments, not production transformer evaluations.
...
| 2 | Stronger standard retrieval tasks | Stage 25 now confirms across five initialization seeds that the long-context learned value-output path favors RoPE-like scoring. Next, move to trained small-transformer Needle-in-Haystack, RULER-style multi-hop/aggregation, or compact natural-language QA tasks with matched compute and multiple seeds. |
| 3 | Comparable PhaseWrap mechanism | Stage 13 through Stage 25 now test adapters around PhaseWrap features and show that explicit distance terms are useful for positional ranking but not sufficient for the learned long-context value-output path. Stage 25 preserves the current gap versus RoPE-like scoring. |

...
| 2 | Stronger standard retrieval tasks | Stage 26 adds an explicit content-key QA retrieval packet, Stage 27 trains a compact attention bridge on that packet, Stage 28 trains a compact attention bridge on non-phase-cued RULER-style retrieval rows, Stage 29 shows period-pair swaps alone do not solve the fixed-score retrieval gap, Stage 30 equalizes the compact bridge feature/parameter budget, Stage 31 shows the current simple PhaseWrap variants fail a harder full-prefix retrieval-attention stress test, and Stage 32 shows a richer feature bridge can recover argmax retrieval. Next, move this structure into a trained small-transformer Needle-in-Haystack, RULER-style multi-hop/aggregation, or compact natural-language QA task with matched compute and multiple seeds. |
| 3 | Comparable PhaseWrap mechanism | Stage 13 through Stage 32 now test adapters, period-pair alternatives, a matched feature-budget bridge, full-context retrieval attention, and a nonlinear full-context feature bridge around PhaseWrap features and show mixed behavior: useful ranking under richer bridges, but still weaker probability/calibration than RoPE-like scoring. |

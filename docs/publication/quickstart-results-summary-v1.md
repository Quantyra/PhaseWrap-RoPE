...
| Next downstream gate | Stronger decoder-only transformer | Replace or materially strengthen the decoder-only implementation beyond the current pointer-generator enough to generalize on retrieval tasks, then rerun the same fair comparison. |

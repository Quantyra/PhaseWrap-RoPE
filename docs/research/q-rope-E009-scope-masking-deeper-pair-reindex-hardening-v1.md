# Q-RoPE E009 Scope-Masking Deeper Pair-Reindex Hardening v1

Date: 2026-03-17
Stories: S1563-S1566

## Fixed Packet
- dataset: `synthetic_positional_scope_masked_reference_selection_response`
- perturbation: `pair_reindex=7`
- witness: `V_future_relational_witness_positional_scope_masked_reference_selection`
- control: `V_control_symbolic_positional_scope_masked_reference_selection_regressor`
- backend: `sim_quantum_statevector`
- seeds: `42`, `123`, `777`

## Mean Packet Results
- witness:
  - `mae = 0.017703`
  - `rank_correlation = 0.406875`
  - `calibration_slope = 0.959800`
- control:
  - `mae = 0.019859`
  - `rank_correlation = 0.121797`
  - `calibration_slope = 0.593794`

## Interpretation
- `pair_reindex=7` was non-inert.
- The witness remained ahead on both declared mean packet metrics.
- The scope-masked line survived the full retained hardening cycle.

## Summary CSV
- `logs/ablation_runs/summary/E009_scope_masking_pair7_v1.csv`

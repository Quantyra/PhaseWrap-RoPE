# Q-RoPE Internal Result Package v5

Date: 2026-03-12

## Standing Benchmark
- `V_future_relational_witness_symbolic_insufficiency`
- benchmark task:
  - `synthetic_symbolic_insufficiency_transition_response`
- strongest bounded symbolic reviews did not catch up across the frozen symbolic-family ladder.

## Preserved Positive Transfer Results
- path-local transfer:
  - `V_future_relational_witness_symbolic_insufficiency_path`
  - preserved after bounded hardening
- loop-closure transfer:
  - `V_future_relational_witness_symbolic_insufficiency_loop`
  - preserved after bounded hardening
- fork-join transfer:
  - `V_future_relational_witness_symbolic_insufficiency_fork_join`
  - preserved after bounded hardening
- relay-binding transfer:
  - `V_future_relational_witness_symbolic_insufficiency_relay_binding`
  - preserved after bounded hardening

## Archived Transfer Evidence
- braid transfer:
  - `V_future_relational_witness_symbolic_insufficiency_braid`
  - valid bounded positive first packet
  - archived after the bounded symbolic control caught up on the deeper structural hardening packet

## Supported Internal Claim
- The witness advantage is not confined to one synthetic benchmark.
- It transfers across four materially different bounded relational task families under frozen symbolic fairness rules.
- The repo now also has one archived transfer failure boundary, which improves internal interpretability.
- The result remains internal-only and mechanism-level.

## Not Supported
- no hardware claim
- no NISQ claim
- no product claim
- no publication-ready superiority claim

## Recommended Use
- preserve as the internal benchmark package for future review
- preserve the transfer portfolio as:
  - positive bounded evidence: `path`, `loop-closure`, `fork-join`, `relay-binding`
  - archived negative boundary: `braid`
- only reopen execution for:
  - a materially different transfer family, or
  - a theory-backed fairness family that is not another same-family escalation

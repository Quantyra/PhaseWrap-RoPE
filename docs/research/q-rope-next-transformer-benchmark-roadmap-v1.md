...
The next Stage 43 gate should be claim-boundary driven: harden the trainable pointer-generator output path or move the same fair-comparison frame into a stronger matched decoder-only transformer. Hardware witness hardening remains a separate replication track and should not displace the fair-comparison promotion path.

...
## Result
Stage 128 adds guarded provider SDK client factory boundaries for IBM Runtime and Amazon Braket.
...
## What this supports
- Provider adapters expose non-secret client configuration metadata.
...
- The future real SDK client implementation has a single boundary that must preserve the Stage 127 injected-client execution contract.
...
## Next gate
After Stage 106/111 readiness clears, replace the blocked client factories with real SDK client creation while preserving the Stage 127 injected-client execution contract.

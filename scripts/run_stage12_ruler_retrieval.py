...
from qrope.stage12_ruler_retrieval import (  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run deterministic Stage 12 local RULER-style retrieval benchmark.")
    ...
    result = run_stage12_benchmark(
        seeds=args.seeds,
        context_lengths=args.context_lengths,
        examples_per_task_length=args.examples_per_task_length,
    )
    paths = write_stage12_outputs(result, args.output_dir)
    print_stage12_table(result)
    print(f"best_method_by_top1_mrr: {result['best_method_by_top1_mrr']}")
    print(
        "phasewrap_oracle_target_overlap: "
        f"{result['phasewrap_target_diagnostic']['phasewrap_oracle_target_overlap']}"
    )

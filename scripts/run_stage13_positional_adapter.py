...
from qrope.stage13_positional_adapter import (  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run deterministic Stage 13 positional-adapter benchmark.")
    ...
    result = run_stage13_benchmark(
        seeds=args.seeds,
        context_lengths=args.context_lengths,
        examples_per_task_length=args.examples_per_task_length,
        epochs=args.epochs,
    )
    paths = write_stage13_outputs(result, args.output_dir)
    print_stage13_table(result)

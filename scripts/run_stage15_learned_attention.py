...
from qrope.stage15_learned_attention import (  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run deterministic Stage 15 learned attention-readout benchmark.")
    ...
    result = run_stage15_benchmark(
        seeds=args.seeds,
        context_lengths=args.context_lengths,
        examples_per_task_length=args.examples_per_task_length,
        epochs=args.epochs,
    )
    paths = write_stage15_outputs(result, args.output_dir)
    print_stage15_table(result)

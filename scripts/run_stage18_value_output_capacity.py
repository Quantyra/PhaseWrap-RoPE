...
from qrope.stage18_value_output_capacity import (  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run deterministic Stage 18 value-output capacity probe.")
    ...
    result = run_stage18_benchmark(
        seeds=args.seeds,
        context_lengths=args.context_lengths,
        examples_per_task_length=args.examples_per_task_length,
        epochs=args.epochs,
    )
    paths = write_stage18_outputs(result, args.output_dir)
    print_stage18_table(result)

...
from qrope.stage88_structural_retrieval_routed_copy_expert_audit import (  # noqa: E402
    DEFAULT_AUDIT_SEEDS,
    DEFAULT_EPOCHS,
    DEFAULT_EXAMPLES_PER_LENGTH,
    DEFAULT_EXACT_OFFSET_ROUTE_WEIGHT,
    DEFAULT_LEARNING_RATE,
    DEFAULT_OUTPUT_DIR,
    DEFAULT_SUPPORT_AUX_WEIGHT,
    DEFAULT_SUPPORT_ROUTE_WEIGHT,
    DEFAULT_TARGET_ATTENTION_AUX_WEIGHT,
    print_stage88_summary,
    run_stage88_audit,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Stage 88 structural retrieval-routed copy expert audit.")
    ...
    parser.add_argument("--support-route-weight", type=float, default=DEFAULT_SUPPORT_ROUTE_WEIGHT)
    parser.add_argument("--exact-offset-route-weight", type=float, default=DEFAULT_EXACT_OFFSET_ROUTE_WEIGHT)
    ...
    result = run_stage88_audit(
        ...
        support_route_weight=args.support_route_weight,
        exact_offset_route_weight=args.exact_offset_route_weight,
    )
    paths = write_stage88_outputs(result, args.output_dir)
    print_stage88_summary(result)

...
STAGE183_SCHEMA_VERSION = "qrope_stage183_contrast_amplified_delta_candidate_screen_v1"
...
DEFAULT_OUTPUT_DIR = DEFAULT_ARTIFACT_ROOT / "stage183_contrast_amplified_delta_candidate_screen"
DESIGN_FAMILY_ID = "pw_contrast_amplified_delta_v1"
...
def _contrast_components_for_family(family: str, delta: float, max_abs_delta: float) -> tuple[float, float]:
    normalized = _normalized_delta(delta, max_abs_delta)
    amplified = _clamp(1.6 * normalized, -1.0, 1.0)
    phase = math.pi * amplified
    if family == "phasewrap":
        return _clamp(math.sin(phase)), _clamp(math.sin(0.75 * phase))
    if family == "rope_like":
        return _clamp(math.cos(0.35 * phase)), _clamp(math.cos(0.175 * phase))
    if family == "sinusoidal_like":
        return _clamp(math.sin(2.25 * phase)), _clamp(math.sin(1.25 * phase))
    if family == "alibi_like":
        component = _clamp(1.0 - 2.0 * abs(amplified))
...
def _candidate_row(source_row: dict[str, Any], family: str, max_abs_delta: float, circuit_template: str) -> dict[str, Any]:
    ...
    component_a, component_b = _contrast_components_for_family(family, delta, max_abs_delta)
...
def _packet_for_lane_family(lane: SourceLane, family: str, circuit_template: str) -> dict[str, Any]:
    ...
    packet_core = {
        "schema_version": STAGE183_SCHEMA_VERSION,
        "packet_version": "qrope_stage183_contrast_amplified_delta_candidate_packet_v1",
        ...
        "matching_policy": {
            ...
            "component_transform": "bounded 1.6x normalized delta amplification applied before RY preparation",
        ...
        "claim_boundary": (
            "Contrast-amplified delta candidate packet for simulated IBM-informed screening only; "
...
def run_stage183_contrast_amplified_delta_candidate_screen(
    *,
    ...
    if isinstance(stage181, dict):
        ...
        if DESIGN_FAMILY_ID not in family_ids:
            blockers.append("stage181_contrast_amplified_delta_family_missing")
    ...
    if not blockers:
        with tempfile.TemporaryDirectory(prefix="qrope_stage183_packets_") as temp_dir:
    ...
    if blockers:
        decision = "CONTRAST_AMPLIFIED_DELTA_CANDIDATE_SCREEN_INCOMPLETE"
    elif reopen["supports_hardware_reopen"]:
        decision = "CONTRAST_AMPLIFIED_DELTA_CANDIDATE_SUPPORTS_HARDWARE_REOPEN"
    else:
        decision = "CONTRAST_AMPLIFIED_DELTA_CANDIDATE_DOES_NOT_SUPPORT_HARDWARE_REOPEN"

    return {
        "schema_version": STAGE183_SCHEMA_VERSION,
        "stage": "stage183_contrast_amplified_delta_candidate_screen",
        ...
        "claim_boundary": {
            "supported": [
                "simulated-only second redesign screen for the Stage181 contrast-amplified delta family",
...
def write_stage183_outputs(result: dict[str, Any], output_dir: Path = DEFAULT_OUTPUT_DIR) -> dict[str, str]:
...
def print_stage183_summary(result: dict[str, Any]) -> None:

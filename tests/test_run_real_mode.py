...
def test_positional_scope_masked_reference_selection_witness_backend_runs() -> None:
    metrics = run_real_experiment(
        dataset="synthetic_positional_scope_masked_reference_selection_response",
        seed=42,
        backend="sim_quantum_statevector",
        variant="V_future_relational_witness_positional_scope_masked_reference_selection",
    )
    diagnostics = metrics["dataset_diagnostics"]
    assert metrics["data_mode"].startswith(
        "synthetic_positional_scope_masked_reference_selection_response+readout_relational_witness_positional_scope_masked_reference_selection+head_linear"
    )
    assert diagnostics["coarse_scope_masking_state_null_pass"] is True
    assert diagnostics["within_scope_masking_state_variation_pass"] is True
    assert diagnostics["in_scope_target_nontrivial_pass"] is True
    assert diagnostics["out_of_scope_distractor_nontrivial_pass"] is True
    assert diagnostics["scope_only_null_pass"] is True
    assert diagnostics["content_only_null_pass"] is True
    assert diagnostics["position_only_null_pass"] is True
    assert diagnostics["final_target_nontrivial_pass"] is True
    assert diagnostics["candidate_set_nontrivial_pass"] is True
    ...
    run_diagnostics = metrics["run_diagnostics"]
    assert run_diagnostics["bounded_feature_audit_pass"] is True
    assert run_diagnostics["forbidden_scope_masking_feature_family_absent_pass"] is True


def test_positional_scope_masked_reference_selection_symbolic_control_freezes_basis() -> None:
    metrics = run_real_experiment(
        dataset="synthetic_positional_scope_masked_reference_selection_response",
        seed=42,
        backend="sim_quantum_statevector",
        variant="V_control_symbolic_positional_scope_masked_reference_selection_regressor",
    )
    diagnostics = metrics["run_diagnostics"]
    assert diagnostics["allowed_scope_masking_symbolic_basis_frozen_pass"] is True
    assert diagnostics["forbidden_scope_masking_feature_family_absent_pass"] is True
...
def test_positional_clause_intersection_reference_selection_witness_backend_runs() -> None:
    metrics = run_real_experiment(
        dataset="synthetic_positional_clause_intersection_reference_selection_response",
        seed=42,
        backend="sim_quantum_statevector",
        variant="V_future_relational_witness_positional_clause_intersection_reference_selection",
    )
    diagnostics = metrics["dataset_diagnostics"]
    assert metrics["data_mode"].startswith(
        "synthetic_positional_clause_intersection_reference_selection_response+readout_relational_witness_positional_clause_intersection_reference_selection+head_linear"
    )
    assert diagnostics["coarse_clause_intersection_state_null_pass"] is True
    assert diagnostics["within_clause_intersection_state_variation_pass"] is True
    assert diagnostics["clause_one_only_null_pass"] is True
    assert diagnostics["clause_two_only_null_pass"] is True
    assert diagnostics["joint_intersection_nontrivial_pass"] is True
    assert diagnostics["candidate_set_nontrivial_pass"] is True
    assert diagnostics["bounded_candidate_count_pass"] is True
    assert diagnostics["clause_intersection_noncollapse_pass"] is True
    assert diagnostics["token_view_balance_pass"] is True
    assert diagnostics["final_target_nontrivial_pass"] is True
    run_diagnostics = metrics["run_diagnostics"]
    assert run_diagnostics["bounded_feature_audit_pass"] is True
    assert run_diagnostics["forbidden_clause_intersection_feature_family_absent_pass"] is True


def test_positional_clause_intersection_reference_selection_symbolic_control_freezes_basis() -> None:
    metrics = run_real_experiment(
        dataset="synthetic_positional_clause_intersection_reference_selection_response",
        seed=42,
        backend="sim_quantum_statevector",
        variant="V_control_symbolic_positional_clause_intersection_reference_selection_regressor",
    )
    diagnostics = metrics["run_diagnostics"]
    assert diagnostics["allowed_clause_intersection_symbolic_basis_frozen_pass"] is True
    assert diagnostics["forbidden_clause_intersection_feature_family_absent_pass"] is True
    assert diagnostics["single_symbolic_family_across_clause_patterns_pass"] is True

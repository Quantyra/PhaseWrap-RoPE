...
from qrope.stage100_matched_cx_encoding_packet_freezer import run_stage100_freezer, write_stage100_outputs
from qrope.stage99_matched_fixed_width_encoding_packet_freezer import ENCODING_FAMILIES
...
def test_stage100_freezes_matched_cx_families_for_each_source_lane(tmp_path) -> None:
    ...
    result = run_stage100_freezer(source_packet_dir=tmp_path, source_packet_files=("lane_a.json", "lane_b.json"))

    assert result["stage"] == "stage100_matched_cx_encoding_packet_freezer"
    assert result["status"] == "completed"
    assert result["decision"] == "MATCHED_CX_ENCODING_PACKETS_FROZEN_NO_HARDWARE"
    ...
    for packet in result["packets"]:
        ...
        assert packet["fixed_width"]["entangling_gate"] == "cx q0->q1"
        assert packet["fixed_width"]["circuit_template"] == "two_ry_cx_parity_z_readout_v1"
        assert packet["row_count"] == 2
        assert packet["execution_status"] == "not_submitted"
        assert "cnot q[0], q[1];" in packet["rows"][0]["openqasm3"]


def test_stage100_no_position_control_is_centered_with_cx_template(tmp_path) -> None:
    _write_packet(tmp_path, "lane_a.json", _source_packet("lane_a"))

    result = run_stage100_freezer(source_packet_dir=tmp_path, source_packet_files=("lane_a.json",))
    ...
    for row in control["rows"]:
        ...
        assert row["circuit_parameters"]["template"] == "two_ry_cx_parity_z_readout_v1"


def test_stage100_outputs_are_written(tmp_path) -> None:
    _write_packet(tmp_path, "lane_a.json", _source_packet("lane_a"))
    result = run_stage100_freezer(source_packet_dir=tmp_path, source_packet_files=("lane_a.json",))

    paths = write_stage100_outputs(result, tmp_path / "out")

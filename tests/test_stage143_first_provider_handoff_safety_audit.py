...
def _stage142(
    path,
    *,
    unsafe_template: bool = False,
    unsafe_command: bool = False,
    extra_template_key: bool = False,
    stage139_context_blockers: list[str] | None = None,
) -> None:
    ...
    _write_json(
        path,
        {
            ...
            "stage139_context_blockers": stage139_context_blockers or [],
...
def test_stage143_verifies_placeholder_and_non_live_handoff(tmp_path) -> None:
    ...
    assert result["stage139_context_ready"] is True
...
def test_stage143_blocks_stage139_context_blockers(tmp_path) -> None:
    stage142 = tmp_path / "stage142.json"
    _stage142(stage142, stage139_context_blockers=["stage139_action_checklist_not_ready"])

    result = run_stage143_audit(stage142_results_path=stage142)

    assert result["decision"] == "FIRST_PROVIDER_HANDOFF_SAFETY_BLOCKED"
    assert result["stage139_context_ready"] is False
...
def test_stage143_outputs_are_written(tmp_path) -> None:
    ...
    assert manifest["stage139_context_ready"] is True
